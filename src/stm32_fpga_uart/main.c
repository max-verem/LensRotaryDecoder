...
int main(int argc, char** argv)
{
    ...
    if(!r)
    {
        ...
        /* run */
        pthread_create(&th, 0, stm32_fpga_uart_proc, instance);
...
//        ws_server_run(instance);
        ...
        while(!f_exit)
        {
            if(!instance->f_demonize)
                fprintf(stderr, "| %10d | %10d | %10d | %10" PRId64 " | %10" PRId64 " | %10" PRId64 " |\r",
                    instance->values[0], instance->values[1], instance->values[2],
                    instance->recv[0], instance->recv[1], instance->recv[2]);
