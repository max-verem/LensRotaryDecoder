...
{
	uint8_t buf[] = {0x00, C};
    return HAL_I2C_Master_Transmit(&hi2c1, ctx->addr << 1, buf, 2, 100);
};
...
static void SSD1306_cb_complete(SSD1306_ctx_t *ctx)
{
	...
	switch(ctx->state)
	{
		...
		case SSD1306_FB_POS0:
			...
			r = HAL_I2C_Master_Transmit_DMA(ctx->hi2c, ctx->addr << 1, ctx->fb[ctx->flip], 1024 + 1);
			break;
...
#define SSD1306_DEF(I2C_NAME, CTX_NAME, ADDR_VAL, WIDTH, HEIGHT) \
...
{
	/* primary setup */
	SSD1306_CMD(ctx, SSD1306_DISPLAYOFF);	// turn off display
	SSD1306_CMD(ctx, SSD1306_SETDISPLAYCLOCKDIV);
	SSD1306_CMD(ctx, 0x80);
	SSD1306_CMD(ctx, SSD1306_SETMULTIPLEX);
	//SSD1306_CMD(ctx, 0x1F);//128×32
	SSD1306_CMD(ctx, 0x3F);//128×64
	SSD1306_CMD(ctx, SSD1306_SETDISPLAYOFFSET);
	SSD1306_CMD(ctx, 0x00);
	SSD1306_CMD(ctx, SSD1306_SETSTARTLINE | 0x00);//0
	SSD1306_CMD(ctx, SSD1306_CHARGEPUMP);
	SSD1306_CMD(ctx, 0x14);
	SSD1306_CMD(ctx, SSD1306_MEMORYMODE);
	SSD1306_CMD(ctx, 0x00);
	SSD1306_CMD(ctx, SSD1306_SEGREMAP | 0x1);
	SSD1306_CMD(ctx, SSD1306_COMSCANDEC);
	SSD1306_CMD(ctx, SSD1306_SETCOMPINS);
	//SSD1306_CMD(ctx, 0x02);//128×32
	SSD1306_CMD(ctx, 0x12);//128×64
	SSD1306_CMD(ctx, SSD1306_SETCONTRAST);// Max contrast
	//SSD1306_CMD(ctx, 0x0F);//0xCF
	SSD1306_CMD(ctx, 0xCF);//0xCF
	SSD1306_CMD(ctx, SSD1306_SETPRECHARGE);
	SSD1306_CMD(ctx, 0xF1);
	SSD1306_CMD(ctx, SSD1306_SETVCOMDETECT);
	//SSD1306_CMD(ctx, 0x10);//0x40
	SSD1306_CMD(ctx, 0x40);//0x40
	SSD1306_CMD(ctx, SSD1306_DISPLAYALLON_RESUME);
	SSD1306_CMD(ctx, SSD1306_NORMALDISPLAY); // Нормальный режим (не инверсный)
	SSD1306_CMD(ctx, SSD1306_DISPLAYON);// Включаем дисплей
